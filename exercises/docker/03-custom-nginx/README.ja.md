...
### Q. ターミナル上で Docker デーモンが起動しているか確認する方法はある？

A. あります。macOS（Docker Desktop 等）環境では、OSのシステム管理コマンド（`systemctl` など）ではなく、**Docker クライアントからの疎通確認**を行うのが最も確実です。

代表的な確認用コマンドは以下の 2 つです。

1. **`docker version` (おすすめ)**
   - 正常時: `Client:` と `Server:` の両方のバージョン情報が表示される。
   - 停止時: `Client:` は表示されるが、`Server:` の部分で `error during connect` となる。エラーを通して「クライアント・サーバーモデル」がはっきりと確認できる。

2. **`docker info`**
   - 正常時: コンテナ数やイメージ数、ストレージのパスなど、デーモンの詳細な稼働状態がズラッと出力される。
   - 停止時: エラーになり接続できない旨が表示される。

> 補足: もし Linux 環境（Ubuntuなど）で直接 Docker を動かしている場合は、OS自体がデーモンを管理しているため `systemctl status docker` というコマンドでプロセスの稼働状態を確認できます。
