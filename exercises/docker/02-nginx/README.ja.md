...
この演習を十分に理解したかの自己評価チェックリスト。以下に ✅ できれば、演習 03 に進んでよい。

- [ ] **実行できた**
  - `docker run --rm -p 8080:80 -v ...` コマンドが正常に実行できた
  - ターミナルにエラーが出ていない

- [ ] **ブラウザで確認できた**
  - `localhost:8080` にアクセスして `index.html` の内容が表示された
  - 期待したスタイル、テキスト、ポート表記が全て見える

- [ ] **3 層の役割が説明できる**
  - 「ホスト側の 8080」の役割を説明できる
  - 「コンテナ側の 80」の役割を説明できる
  - 「Docker のポート転送」がどう繋いでいるか説明できる

- [ ] **bind mount を理解できた**
  - `-v` オプションが「ファイルをコンテナに見せる」ことを理解している
  - 「ホスト側ファイル : コンテナ側パス」の対応を説明できる
  - `:ro` が「読み取り専用」の意味を理解している

- [ ] **トラブル時に切り分けができる**
  - ページが開かない時、どこを見るか判定できる
  - mount の結果を `docker inspect` で確認できた（できれば）
  - コンテナの中身を `docker exec` で確認できた（できれば）

- [ ] **コマンドの使い分けが理解できた**
  - `docker ps` で running container を確認
  - `docker logs` で container のエラーを確認
  - `docker inspect` で mount や port を確認
  - `docker exec` で container の中を見る

- [ ] **演習 03 へのターニングポイントを理解している**
  - 演習 02 は「既成イメージ + bind mount = 開発向き」
  - 演習 03 は「Dockerfile build = 本番向き」
  - その違いを説明できる

これらが全部 ✅ なら、演習 03 に進んで OK。

いくつか ❌ があれば、該当する Q&A セクションを読み直すか、コマンドをもう一度試してみることを推奨。
