...
- Q. Where does the hash suffix in a Pod name (like `hello-nginx-d54d8664b-6dhqx`) come from?
  - A. `d54d8664b` is the ReplicaSet ID (auto-generated by Deployment), and `6dhqx` is a random suffix per Pod. Find the full name in `kubectl get pods` output.
  - Details: `exercises/k8s/01-deployment/README.md`

- Q. What is the difference between `kubectl logs`, `kubectl describe pod`, and `journalctl`?
  - A. Three logging layers: (1) `kubectl logs` = container stdout/stderr, (2) `kubectl describe pod` = Pod lifecycle events, (3) `journalctl` = node OS-level kubelet/containerd logs. First two solve ~99% of support cases.
  - Details: `exercises/k8s/01-deployment/README.md`

- Q. Where are logs actually stored? Does `kubectl logs` read from files on the node?
  - A. Kubernetes captures container stdout/stderr as streams, not files. Logs are kept in `/var/log/pods/` on nodes temporarily, but this is implementation detail; `kubectl logs` abstracts this via API.
  - Details: `exercises/k8s/01-deployment/README.md`
...
- Q. What is `k3s` and how is it different from `kind`?
  - A. `k3s` is a lightweight Kubernetes distribution designed for edge and low-resource environments. `kind` is a local cluster tool that runs upstream Kubernetes in Docker containers. Both support standard Kubernetes APIs, but k3s is a packaged distribution and kind is a cluster creation tool.
  - Details: `notes/k3s-basics.md`
